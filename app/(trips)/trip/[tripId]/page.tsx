...
export default function TripInfoPage() {
  ...
  const navLinks = [{ label: "Trips", href: "/trips" }];
  ...
  useEffect(() => {
    ...
    const fetchTripData = async () => {
      ...
      try {
        const tripResponse = await fetch(`http://localhost:8080/trips/trip/${tripId}`, {
          method: "GET",
        ...
        const activityListsResponse = await fetch(`http://localhost:8080/activities/lists/${tripId}`, {
          method: "GET",
          headers: { Authorization: `Bearer ${token}` },
        });
        ...
        const budgetSummariesResponse = await fetch(`http://localhost:8080/budgets/${tripId}/summaries`, {
          method: "GET",
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  {budgetSummaries.length > 0 ? (
    <ul>
      {budgetSummaries.map((summary) => (
        <li key={summary.name} className="text-white mb-4">  
          <p>
            <h1>{summary.name}:</h1>  {summary.total}
          </p>
          <p>
            Money Spent: {summary.moneySpent}
          </p>
        </li>
      ))}
    </ul>
